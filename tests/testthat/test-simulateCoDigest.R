...
test_that("simulateCoDigest works as intended with valid inputs", {
  expect_no_error(
    testDigestDF <- simulateCoDigest(Biostrings::DNAString("CGGCCGATCGATCGGCCG"),
                                     Enzymes[c(1,4),])
    )
  expectedResult <- tibble::tibble(Enzymes = rep(c("AaaI + AagI"), times = 4),
                                   ...
                                   End = c(1, 8, 13, 17),
                                   Length = c(1, 7, 5, 4))
  expect_equal(testDigestDF, expectedResult)
})

test_that("simulateCoDigest informs user when digestion results in no cleavage sites", {
  expect_message(

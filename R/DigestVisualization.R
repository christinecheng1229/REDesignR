...
#' Visualize restriction enzyme digestion patterns as linear maps
#'
#' This function produces one or more *linear restriction maps* from the output
#' of [simulateCoDigest()]. A restriction map shows fragment positions along the
#' original DNA sequence.
#'
#' @param codigestDf A tibble or data frame of co-digestion results in the
#'  format outputted by [simulateCoDigest()].
#' @param multiDigest A boolean indicating whether `codigestDf` resulted from a
#'  single-digest (`FALSE`) or a co-digest (`TRUE`), the latter being the default.
#' @param showLengths A boolean dictating whether numerical fragment lengths are
#' shown as overlay on the plot; TRUE by default.
#'
#' @returns
#' For single-digest input (`multiDigest = FALSE`), the function returns a single
#' plot representing the fragment layout for that enzyme.
#'
#' For co-digest input (`multiDigest = TRUE`), three restriction maps are
#' generated and stacked:
#'   1. The co-digest (combined enzymes)
#'   2. The first enzyme alone
#'   3. The second enzyme alone
#'
#' This allows visual comparison between single- and co- digestion patterns.
#'
#' @examples
#' # get sample data
...
#'
#' # Example 1: ---------------------------------------------------------------
#' # Co-digest visualization
#' result <- simulateCoDigest(dna, Enzymes[c(1, 4), ])
#' plotRestrictionMap(result$digestDf, multiDigest = TRUE)
#'
#' # hide fragment length labels
#' plotRestrictionMap(result$digestDf, showLengths = FALSE)
#'
#' # Example 2: ---------------------------------------------------------------
#' # Single-enzyme digest visualization
#' single <- simulateCoDigest(dna, Enzymes[c(1, 6), ])
#' plotRestrictionMap(single$digestDf, multiDigest = FALSE)
#'
#' @references
...
#'
#' Pedersen T (2025). _patchwork: The Composer of Plots._
#'   R package version 1.3.2.9000, https://patchwork.data-imaginist.com.
#'
#' Wickham, H. (2016). _ggplot2: Elegant Graphics for Data Analysis._
...
#' Simulate an agarose gel of restriction digestion fragments
#'
#' This function produces a simulated agarose gel diagram from digestion results
#' generated by [simulateCoDigest()].
#'
#' @param codigestDf A tibble or data frame of co-digestion results in the
#'  format outputted by [simulateCoDigest()].
#' @param multiDigest A boolean indicating whether `codigestDf` resulted from a
#'  single-digest (`FALSE`) or a co-digest (`TRUE`), the latter being the default.
#' @param labelFragments  A boolean indicating whether plotted fragments should
#'  be labelled with the _FragmentID_ (`TRUE`) or not (`FALSE`).
#'
#' @examples
#' # get sample data
#' data(Enzymes, package = "REDesignR")
...
#'
#' # Example 1: ---------------------------------------------------------------
#' # Multi-lane gel for a co-digest
#' co <- simulateCoDigest(dna, Enzymes[c(1, 4), ])
#' simulateGel(co$digestDf)
#'
#' # Label fragments
#' simulateGel(co$digestDf, labelFragments = TRUE)
#'
#' # Example 2: ---------------------------------------------------------------
#' # Single-digest gel
#' single <- simulateCoDigest(dna, Enzymes[c(1, 6), ])
#' simulateGel(single$digestDf, multiDigest = FALSE)
#'
#' @returns A plot representing an agarose gel of the fragments resulting
#'   from [simulateCoDigest()].
#' Each lane corresponds to one digestion condition (single enzyme or co-digest),
#' and each band represents a DNA fragment.
#'
#' Band opacity represents gel band "brightness" which reflects the number of
#' repeated fragment sizes, mimicking real gel intensity variation.
#'
#' If `labelFragments = TRUE`, each band is annotated with the corresponding
#' `FragmentID`'s.
#'
#' @references
#' Wickham H, Pedersen T, Seidel D (2025). _scales: Scale Functions for
#' Visualization._ R package version 1.4.0, https://scales.r-lib.org.
#'
#' @export
#'
#' @import dplyr

...
#' @import tibble
#' @export
simulateCoDigest <- function(dnaSeq, enzymes) {
  ...
  enzymes <- tibble::as_tibble(enzymes)
  ...
  # return digestion tibble with indication of single- or co-digestion
  result <- list(
    digestDf   = digestDf,
...
# Helper function that builds digest tibble to reduce repeated code chunks.
